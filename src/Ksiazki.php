...
class Ksiazki
{
	...
	{
		$sql = "SELECT k.*, a.nazwisko, a.imie, sum(zs.liczba_sztuk) 
                FROM ksiazki k 
                JOIN autorzy as a ON k.id_autora=a.id 
                LEFT JOIN zamowienia_szczegoly zs on zs.id_ksiazki = k.id
                GROUP BY k.tytul, a.imie, a.nazwisko, k.zdjecie
                ORDER BY sum(zs.liczba_sztuk) desc limit 5";
